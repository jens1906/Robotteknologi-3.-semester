...
def get_enhanced_image(ImagePath):
    ...
    #get image without adding noise
    img = cv.imread(ImagePath)
    img = cv.cvtColor(img, cv.COLOR_BGR2RGB)
    img_ = cv.resize(img,(500,500))
...
InputPath="LightEnhance/wm-nowm/"
...
if user_input == "y":
    ...
    Model_Enhancer.load_weights("LightEnhance/Enhancer.h5")

#Take a random image from ExDark dataset
test_image = "LightEnhance/ConvolutionalNeuralNetwork/MadsGonster.jpg"
...
#test the model
TestPath="LightEnhance/ConvolutionalNeuralNetwork/"
ImagePath = TestPath+"MadsGonster.jpg"
image_for_test = ExtractTestInput(ImagePath)
Prediction = Model_Enhancer.predict(image_for_test)
